...
const updateAll = instruments => new Promise((resolve, reject) => {
  const recurse = (array) => {
    if (!array.length) {
    ...
    } else if (instrument.name.length) {
      const newInstrument = new Instrument({ name: instrument.name, image: instrument.image });
...
module.exports = {
  Instrument, getAll, updateAll, deleteMany,
};
