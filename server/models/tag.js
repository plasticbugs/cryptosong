...
*/
module.exports.getSongsForTag = name => new Promise((resolve, reject) => {
  Tag.find({ name })
    .then((results) => {
      resolve(results[0]);
    });
...
*/
module.exports.addOrInsertTags = (tagArray, forSongId, task) => new Promise((resolve, reject) => {
  const number = forSongId;
  let counter = tagArray.length;
  for (const tag of tagArray) {
    const num = tag._id.toString();
    if (num.match(/^[0-9a-fA-F]{24}$/)) {
      ...
      const newTag = new Tag({ name: tag.name, image: tag.image });
      SongModel.Song.findOne({ number })
        .then((song) => {
          newTag.save()
            .then((savedTag) => {
...
*/
module.exports.insertTag = (tagData, forSongId) => new Promise((resolve, reject) => {
  console.log(tagData, forSongId);
  if (forSongId) {
    const song = SongModel.Song.findOne({ number: Number.parseInt(forSongId, 10) })
      .then(song => song)
      .then((song) => {
        ...
        tag.save((err, newTag) => {
          ...
          const tagForSong = cleanObj(newTag);
...
const deleteTags = tags => new Promise((resolve, reject) => {
...
*/
module.exports.updateAll = tagArray => new Promise((resolve, reject) => {
  const recurse = (array) => {
    if (!array.length) {
      Tag.find({})
        .select('_id name image')
