...
export default class Collection extends Component {
  ...
  handleChange(e, { name, value, id }) {
    ...
    if (item.length > 0) {
      ...
      let collection = [...this.state.collection.slice(0, ind), item, ...this.state.collection.slice(ind + 1)]
  ...
  addNewItem(e) {
    ...
    let collection = [item, ...this.state.collection]
    this.setState({collection});
  }
  ...
  removeItemsFromDB(cb) {
    let deleteThese = this.state.markedForDeletion.filter(id => {
  ...
  handleDelete(e, id) {
    ...
    let index = this.state.collection.indexOf(item[0]);
    let state = this.state.collection;
    let markedForDeletion = this.state.markedForDeletion;
    this.setState({
      ...
      <Container>
        ...
        <Form onSubmit={this.handleSubmit}>
          <List>{collection.map((item) => {
            return this.renderItemFields(item);
          })}</List>
          <Grid columns={2}>
            <Grid.Row>
              <Grid.Column>
